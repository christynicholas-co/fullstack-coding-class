...
SELECT t.id AS team_id, t.name, t.city, t.state, p.fname ||' '|| p.lname AS fullname, p.weight,
    CASE
        WHEN p.weight > 0
            AND p.weight >= 200 THEN 'heavy'
        WHEN p.weight > 0
            AND p.weight < 200 THEN 'medium'
        END duration
FROM hr.players AS p
RIGHT JOIN hr.teams AS t
ON p.team_id = t.id
ORDER BY p.weight ASC;

Rescore - as I gave you an answer, I am only giving you 50%
But I am glad you went and worte it in your way AND the comments. 
This is great!!!
Rescore - 50%
